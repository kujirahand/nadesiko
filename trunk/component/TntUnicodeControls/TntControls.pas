...
{ Unicode enable .OnKeyPress event }
...
begin
  if csDestroying in AOwner.ComponentState then begin
    {$IFNDEF COMPILER_10_UP}
    if not (csDesigning in AOwner.ComponentState) then
    {$ENDIF}
      raise ETntInternalError.Create('TNT Internal Error: Can not subclass a control while it''s being destroyed: ' + AOwner.Name + ' (' + AOwner.Classname + ')');
  end;
