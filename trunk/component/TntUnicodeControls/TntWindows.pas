...
begin
  ...
  WStrPCopy{TNT-ALLOW WStrPCopy}(WideFindData.cFileName, AnsiFindData.cFileName);
  WStrPCopy{TNT-ALLOW WStrPCopy}(WideFindData.cAlternateFileName, AnsiFindData.cAlternateFileName);
end;
...
begin
  if Win32PlatformIsUnicode then
    ...
    Result := GetVolumeInformationA{TNT-ALLOW GetVolumeInformationA}(PAnsiChar(lpRootPathName), PAnsiChar(lpVolumeNameBuffer), nVolumeNameSize, lpVolumeSerialNumber, lpMaximumComponentLength, lpFileSystemFlags, PAnsiChar(AnsiBuff), AnsiBuffLen);
    if Result then begin
      SetLength(AnsiBuff, AnsiBuffLen);
      if (nFileSystemNameSize <= AnsiBuffLen) or (Length(AnsiBuff) = 0) then
        Result := False
      else
        WStrPLCopy{TNT-ALLOW WStrPLCopy}(lpFileSystemNameBuffer, AnsiBuff, nFileSystemNameSize);
    end;
  end;
end;
...
begin
  if Win32PlatformIsUnicode then
    ...
    SetLength(AnsiBuff, MAX_PATH);
    SetLength(AnsiBuff, GetTempFileNameA{TNT-ALLOW GetTempFileNameA}(PAnsiChar(lpPathName), PAnsiChar(lpPrefixString), uUnique, PAnsiChar(AnsiBuff)));
    Result := _HandlePathLengthResult(MAX_PATH, lpTempFileName, AnsiBuff, [poZeroSmallBuff]);
  end;
end;
  ...
  if Win32PlatformIsUnicode then
    ...
    if Result then begin
      ...
      if (nSize <= AnsiBuffLen) or (Length(AnsiBuff) = 0) then begin
        ...
        WStrPLCopy{TNT-ALLOW WStrPLCopy}(lpBuffer, AnsiBuff, nSize);
...
begin
  if Win32PlatformIsUnicode then begin
    ...
    if lpbi.pszDisplayName <> nil then
      WStrPCopy{TNT-ALLOW WStrPCopy}(lpbi.pszDisplayName, AnsiInfo.pszDisplayName);
...
begin
  if Win32PlatformIsUnicode then begin
    ...
    if Result then
      WStrPCopy{TNT-ALLOW WStrPCopy}(pszPath, PAnsiChar(AnsiPath))
  end;
end;
...
begin
  if Win32PlatformIsUnicode then begin
    ...
    WStrPLCopy{TNT-ALLOW WStrPLCopy}(psfi.szDisplayName, SHFileInfoA.szDisplayName, MAX_PATH);
    WStrPLCopy{TNT-ALLOW WStrPLCopy}(psfi.szTypeName, SHFileInfoA.szTypeName, 80);
  end;
end;
