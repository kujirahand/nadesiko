...
// Converted to Delphi class 11/04 Toshiki Kumagai
unit BlowFish;
...
type
  TBFWord = DWORD;
  PBFWord = ^TBFWord;
  TBlowFish = class(TCipher)
  private
    PArray: array of DWORD;
    SBoxes: array of array of DWORD;
    procedure Round(var A: TBFWord; const B: TBFWord; N: Integer);
    procedure CustomEncode(XL, XR: TBFWord; var WL, WR: TBFWord);
  ...
  public
    ...
    procedure DecodeValue(const Src; var Dst); override;
    procedure ValueToStream(const Src; var Dst); override;
    procedure StreamToValue(const Src; var Dst); override;
  end;

implementation

const
  MAXKEYBYTES   = 56;   // 448 bits max
  NPASS         = 16;   // SBox passes
...
const
  ...
  RowSize = 256;
  ColSize = 4;
  bf_P: array [0..NPASS2 - 1] of DWORD = (
  ...
  bf_S: array [0..RowSize * ColSize - 1] of DWORD = (
...
type
  TBFWordArray = array [0..MaxInt div SizeOf(TBFWord) - 1] of TBFWord;
  PBFWordArray = ^TBFWordArray;

{ TBlowFish }

procedure TBlowFish.Round(var A: TBFWord; const B: TBFWord; N: Integer);
  function F(X: TBFWord): DWORD;
  var
    A, B, C, D: Byte;
  begin
    A := X shr (OrderIndex0 * 8);
    B := X shr (OrderIndex1 * 8);
    C := X shr (OrderIndex2 * 8);
    D := X shr (OrderIndex3 * 8);

    Result := ((SBoxes[0, A] + SBoxes[1, B]) xor SBoxes[2, C]) + SBoxes[3, D];
  end;
begin
  A := A xor (F(B) xor PArray[N]);
end;

procedure TBlowFish.CustomDecode(XL, XR: TBFWord; var WL, WR: TBFWord);
var
  I: Integer;
begin
  XL := XL xor PArray[17];
...
procedure TBlowFish.CustomEncode(XL, XR: TBFWord; var WL, WR: TBFWord);
var
  I: Integer;
begin
  XL := XL xor PArray[0];
...
begin
  SetLength(PArray, NPASS2);
  SetLength(SBoxes, ColSize, RowSize);
end;

procedure TBlowFish.Initialize(const Key; KeyBytes: Integer);
  function PP(var Value: Integer; Size: Integer): Integer;
  begin
    Result := Value;
    Inc(Value);
    if (Value = Size) then
...
begin
  ...
  K := @Key;
  Move(bf_P[0], PArray[0], NPASS2 * SizeOf(bf_P[0]));

  for I := 0 to ColSize - 1 do
    for J := 0 to RowSize - 1 do
      SBoxes[I, J] := bf_S[I * RowSize + J];

  J := 0;
  for I := 0 to NPASS2 - 1 do
  begin
    Temp := 0;
    Temp := Temp or (K[PP(J, KeyBytes)] shl (OrderIndex0 * 8));
    Temp := Temp or (K[PP(J, KeyBytes)] shl (OrderIndex1 * 8));
    Temp := Temp or (K[PP(J, KeyBytes)] shl (OrderIndex2 * 8));
    Temp := Temp or (K[PP(J, KeyBytes)] shl (OrderIndex3 * 8));

    PArray[I] := PArray[I] xor Temp;
  end;

  DataL := 0;
  DataR := 0;
  for I := 0 to NPASS2 div 2 - 1 do
  begin
    CustomEncode(DataL, DataR, DataL, DataR);
    PArray[2 * I + 0] := DataL;
    PArray[2 * I + 1] := DataR;
  end;
  ...
  for I := 0 to ColSize - 1 do
  begin
    for J := 0 to RowSize div 2 - 1 do
    begin
      CustomEncode(DataL, DataR, DataL, DataR);
      SBoxes[I, 2 * J + 0] := DataL;
      SBoxes[I, 2 * J + 1] := DataR;
    end;
  end;
end;

procedure TBlowFish.DecodeValue(const Src; var Dst);
var
  S, D: PBFWordArray;
begin
  S := @Src;
  D := @Dst;
  CustomDecode(S[0], S[1], D[0], D[1]);
end;

procedure TBlowFish.EncodeValue(const Src; var Dst);
var
  S, D: PBFWordArray;
begin
  S := @Src;
  D := @Dst;
  CustomEncode(S[0], S[1], D[0], D[1]);
end;
...
procedure TBlowFish.StreamToValue(const Src; var Dst);
var
  S: PByteArray;
  D: PBFWord;
begin
...
{$ELSE}
  S := @Src;
  D := @Dst;
  D^ := (S[0] shl (OrderIndex0 * 8)) or (S[1] shl (OrderIndex1 * 8)) or
    (S[2] shl (OrderIndex2 * 8)) or (S[3] shl (OrderIndex3 * 8));
  Inc(D);
  D^ := (S[4] shl (OrderIndex0 * 8)) or (S[5] shl (OrderIndex1 * 8)) or
    (S[6] shl (OrderIndex2 * 8)) or (S[7] shl (OrderIndex3 * 8));
{$ENDIF}
end;

procedure TBlowFish.ValueToStream(const Src; var Dst);
begin
