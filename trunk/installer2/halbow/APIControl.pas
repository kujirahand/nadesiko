...
private
  FHandle: HWND;
  FID: HMenu;
  FOnPopupMenu: TNotifyMessage;                                              // Ç±Ç±
protected
  procedure APICreateParams(var Params: TAPICreateParams); virtual; abstract;
  procedure Initialize; virtual; abstract;
  procedure WMContextMenu(var Msg: TMessage); message WM_CONTEXTMENU;        // Ç±Ç±
public
  ...
  property OnPopupMenu: TNotifyMessage read FOnPopupMenu write FOnPopupMenu;  // Ç±Ç±
end;
...
TAPIStaticText = class(TAPIControl)
private
  FBorder: Boolean;
  ...
  FOnTextChange: TNotifyMessage;
  FOnClick: TNotifyMessage;
  function GetText:string;
  procedure SetText(val:string);
  procedure SetTextAlign(val: TTextAlign);
  procedure SetTextColor(val:COLORREF);
  procedure SetBkColor(val:COLORREF);
  procedure SetBorder(val:Boolean);
  procedure SetSunkenBorder(val:Boolean);
protected
  procedure APICreateParams(var Params: TAPICreateParams); override;
  procedure Initialize; override;
public
  ...
  property SunkenBorder: Boolean read FSunkenBorder write SetSunkenBorder;
  property OnTextChange: TNotifyMessage read FOnTextChange write FOnTextChange;
  property OnClick: TNotifyMessage read FOnClick write FOnClick;
end;
...
procedure TSelfSubClass.WMContextMenu(var Msg: TMessage);
var
  OriginalProc: TFNWndProc;
begin
  if Assigned(FOnPopupMenu) then begin
    Msg.Msg := integer(self); // sender
    FOnPopupMenu(Msg);
  end else begin
    OriginalProc := TFNWndProc(GetProp(FHandle,'WNDPROC'));
    Msg.Result := CallWindowProc(OriginalProc,FHandle,
