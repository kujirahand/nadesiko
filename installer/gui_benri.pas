...
interface
uses
  SysUtils, Classes, Forms, Windows, ShellApi, ComObj, ActiveX, ShlObj,
  Registry;
...
function CommonStartMenuDir:string;
function CommonDesktopDir:string;
...
function SHFileDeleteComplete(const Sorce: string): Boolean;
...
{ソフトを起動する}
function RunApp(const fname: String; bShow: Boolean = True): Boolean;
...
{ソフトを起動する}
function RunApp(const fname: String; bShow: Boolean): Boolean;
var
    retCode: Integer;
begin
    if bShow then
...
function QuickLaunchDir: string;
begin
  Result := AppData + 'Microsoft\Internet Explorer\Quick Launch\';
end;
