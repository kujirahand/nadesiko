...
begin
  ...
  AddFunc  ('ネームスペース変更','Sに|Sへ', 244, sys_namespace_change,   '名前空間をSに変更する','ねーむすぺーすへんこう');
  ...
  AddFunc  ('形式指定','{文字列=?}SをAで',470,  sys_format,     'データSをAの形式で出力する','していけいしき');
  ...
  AddFunc  ('表正規表現ピックアップ','{=?}Aの{=-1}IからSを|Aで', 5067, sys_csv_pickupRegExp,'二次元配列AでI列目(0起点)から正規表現パターンSにマッチする行をピックアップして返す。I=-1で全フィールドを対象にする。','ひょうわいるどかーどぴっくあっぷ');
  ...
  AddFunc  ('クリップボード取得','',  401, sys_getClipbrd,'クリップボードから文字列を取得する','くりっぷぼーど');
  ...
  AddFunc('再生', 'Sを', 498, sys_musPlay,'音楽ファイルSを再生する。','さいせい');
  AddFunc('停止', '',    499, sys_musStop,'「再生」した音楽を停止する。','ていし');
  AddFunc('演奏', 'Sを', 495, sys_musPlay,'音楽ファイルSを演奏する。『再生』と同じ。','えんそう');
  //-MCI
...
begin
  ...
  AddFunc('パックファイルソース実行',  '',        17, sys_packfile_nako_run,   'パックファイルソースロードでロードしたプログラムを実行する。','ぱっくふぁいるそーすろーど');
