...
{  This is Info-Zip's header files
   for the Info-Zip's UnZip32.dll ( version 5.4 )
...
type
  ...
  TWiz_SingleEntryUnzip       = function (ifnc: Integer; ifnv: PPChar; xfnc: Integer;
                                xfnv: PPChar; var Options: TDCL;
                                var UserFunc: TUserFunctions): Integer; stdcall;
  TWiz_UnzipToMemory          = function (Zip: PChar;  FileName: PChar; var UserFunctions: TUserFunctions;
                                var RetStr: TUzpBuffer): Integer; stdcall;
  TWiz_Grep       = function (Archive: PChar; FileName: PChar; Pattern: PChar; Cmd: Integer;
  ...
  { helper }
  TIsExpectedUnZipDllVersion = function : boolean;

var
  Wiz_NoPrinting : TWiz_NoPrinting;
  Wiz_Validate    : TWiz_Validate;
  Wiz_Init        : TWiz_Init;
  Wiz_SetOpts     : TWiz_SetOpts;
  Wiz_Unzip       : TWiz_Unzip;
  Wiz_SingleEntryUnzip: TWiz_SingleEntryUnzip;
  Wiz_UnzipToMemory: TWiz_UnzipToMemory;
  Wiz_Grep        : TWiz_Grep;
  UzpFreeMemBuffer: TUzpFreeMemBuffer;
  UzpVersion      : TUzpVersion;
  UzpVersion2     : TUzpVersion2;
  IsExpectedUnZipDllVersion: TIsExpectedUnZipDllVersion;

// ìÆìIÉçÅ[Éh
function Unzip32Load: Boolean;
function Unzip32Free: Boolean;
...
var Unzip32Handle: THandle = 0;

function Unzip32Load: Boolean;
begin
  Result := (Unzip32Handle <> 0);
  if Result then Exit;
  Unzip32Handle := unit_archive.LoadLibrary(UNZIP_DLL);
  Result := (UnZip32Handle > HINSTANCE_ERROR);;
  if Result = False then
  begin
    Unzip32Handle := 0;
    Exit;
  end;
  Wiz_NoPrinting := GetProcAddress(Unzip32Handle,'Wiz_NoPrinting');
  Wiz_Validate := GetProcAddress(Unzip32Handle,'Wiz_Validate');
  Wiz_Init := GetProcAddress(Unzip32Handle,'Wiz_Init');
  Wiz_SetOpts := GetProcAddress(Unzip32Handle,'Wiz_SetOpts');
  Wiz_Unzip := GetProcAddress(Unzip32Handle,'Wiz_Unzip');
  Wiz_SingleEntryUnzip := GetProcAddress(Unzip32Handle,'Wiz_SingleEntryUnzip');
  Wiz_UnzipToMemory := GetProcAddress(Unzip32Handle,'Wiz_UnzipToMemory');
  Wiz_Grep := GetProcAddress(Unzip32Handle,'Wiz_Grep');
  UzpVersion := GetProcAddress(Unzip32Handle,'UzpVersion');
end;

function Unzip32Free: Boolean;
begin
  if Unzip32Handle <> 0 then
  begin
    Result := FreeLibrary(Unzip32Handle);
    if Result then
    begin
      Unzip32Handle := 0;
      Wiz_NoPrinting := nil;
...
----------------------------------------------------------------------------------}
function IsExpectedUnZipDllVersionEx: boolean;
const
...
begin
  ...
  if iRes = 0 then raise Exception.CreateFmt(DLL_WARNING, [UNZIP_DLL]);
  ...
  then Result :=  (sCompany = COMPANY_NAME) and (sVersion = UNZIP_DLL_VERSION) ;

  if not Result then raise Exception.CreateFmt(DLL_VERSION_WARNING, [UNZIP_DLL]);
