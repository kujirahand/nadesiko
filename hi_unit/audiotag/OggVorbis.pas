...
begin
  ...
  try
    ...
    Source.Seek(Info.ID3v2Size, soFromBeginning);
    Source.Read(Info.FPage, SizeOf(Info.FPage));
    if Info.FPage.ID <> OGG_PAGE_ID then exit;
    Source.Seek(Info.ID3v2Size + Info.FPage.Segments + 27, soFromBeginning);
    { Read Vorbis parameter header }
    Source.Read(Info.Parameters, SizeOf(Info.Parameters));
    if Info.Parameters.ID <> VORBIS_PARAMETERS_ID then exit;
    Info.SPagePos := Source.Position;
    Source.Read(Info.SPage, SizeOf(Info.SPage));
    Source.Seek(Info.SPagePos + Info.SPage.Segments + 27, soFromBeginning);
    Source.Read(Info.Tag.ID, SizeOf(Info.Tag.ID));
    { Read Vorbis tag }
    if Info.Tag.ID = VORBIS_TAG_ID then ReadTag(Source, Info);
    { Get total number of samples }
