...
2002/02/15 ver.1.00 初期作成
2004/10/06 ver.1.10 なでしこ版に変換
2022/01/06 ver.1.11 微修正
--------------------------------------------------------------------------------
*** WildCard ***
...
begin
    ...
    try
        pw.AsString := pattern;
        sw.AsString := JReplace(str,'\','\\');// \ はシーケンスと認識されるので。
        if pw.Match(sw) then
        begin
            ...
            begin
                pickupStr.Text := pw.PickupStr.Text;
            end;
        end else begin
            Result := '';
        end;
...
constructor TWKakko.Create(var p: PChar);
    ...
    begin
        ...
        begin
            m := i; mlen := 999;
            for j:=i to SelectList.Count -1 do  // modified 2004/06/25 by kujira
            begin
